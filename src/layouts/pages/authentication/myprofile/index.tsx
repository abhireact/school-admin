...
const MYProfile = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      console.log(profileResponse, "asdfdf");
      const schoolResponse = await axios.get(`${process.env.REACT_APP_BASE_URL}/mg_school`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
      });
      setSchoolName(schoolResponse.data[0]?.school_name || "");
    } catch (error) {
