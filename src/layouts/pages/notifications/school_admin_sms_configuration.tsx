...
interface GetDataItem {
  url: string;
  support_multiple_sms: boolean;
  maximum_sms_Support: number;
  mobile_number_attribute: string;
  msg_attribute: string;
  sender_id: string;
...
export default function SMSConfigurationCreateSchoolAdmin() {
  const [particularFieldss, setParticularFieldss] = useState([{ key: "", value: "" }]);
  const [getdata, setGetData] = useState<GetDataItem>({
    url: "",
  ...
  const getdataaa = () => {
    axios
      ...
      .then((response) => {
        ...
        console.log(response.data[0].key_value, "dayttatatata");
        setParticularFieldss(response.data[0].key_value);
      })
      .catch(() => {
        console.log("not created configuration");
      });
  };
  useEffect(() => {
    getdataaa();
  ...
  const initialValues = {
    url: createOrEdir == "edit" ? getdata.url : "",
    mobile_number_attribute: createOrEdir == "edit" ? getdata.url : "",
    attribute: createOrEdir == "edit" ? getdata.msg_attribute : "",
    unicode_key: createOrEdir == "edit" ? getdata.unicode_key : "",
    unicode_value: createOrEdir == "edit" ? getdata.unicode_value : "",
    request_type: createOrEdir == "edit" ? getdata.request_type : "",
    sender_id: createOrEdir == "edit" ? getdata.sender_id : "",
    sender_id_value: createOrEdir == "edit" ? getdata.sender_id_value : "",
    vender_name: createOrEdir == "edit" ? getdata.vendor_name : "",
    english_key: createOrEdir == "edit" ? getdata.english_key : "",
    maximum_sms_Support: createOrEdir == "edit" ? getdata.maximum_sms_Support : "",
    english_value: createOrEdir == "edit" ? getdata.english_value : "",
    support_multiple_sms: createOrEdir == "edit" ? getdata.support_multiple_sms : false,
  };
  const { values, errors, touched, handleBlur, handleChange, handleSubmit } = useFormik({
    ...
    onSubmit: async (values, action) => {
      const submitvalue = {
        ...
        key_value: particularFieldss,
        vendor_name: values.vender_name,
      };
      const editvalue = {
        old_url: createOrEdir == "edit" ? getdata.url : "",
        old_sender_id_value: createOrEdir == "edit" ? getdata.sender_id_value : "",
        url: values.url,
        support_multiple_sms: values.support_multiple_sms,
        maximum_sms_Support: values.maximum_sms_Support,
        mobile_number_attribute: values.mobile_number_attribute,
      ...
      if (createOrEdir == "create") {
        axios
          ...
          .then((response) => {
            ...
            setParticularFieldss([{ key: "", value: "" }]);
            action.resetForm();
            getdataaa();
          })
          .catch((error) => {
  ...
  const addParticularField = () => {
    setParticularFieldss([...particularFieldss, { key: "", value: "" }]);
  };

  const removeParticularField = (index: any) => {
    if (index != 0) {
      const updatedFields = [...particularFieldss];
      updatedFields.splice(index, 1);
      setParticularFieldss(updatedFields);
    }
  };
  return (
    <DashboardLayout>
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <Grid xs={12} sm={12} p={2}>
            ...
            {particularFieldss.map((particular: any, index) => (
              <Grid container spacing={3} key={index} p={2}>
                <Grid item xs={10} sm={4}>
                  <MDInput
                    ...
                    value={particularFieldss[index].key}
                    variant="standard"
                    onChange={(e: any) => {
                      const updatedFields = [...particularFieldss];
                ...
                <Grid item xs={10} sm={4}>
                  <MDInput
                    ...
                    sx={{ width: "100%" }}
                    name={`value${index}`}
                    value={particularFieldss[index].value}
                    variant="standard"
                    onChange={(e: any) => {
                      const updatedFields = [...particularFieldss];
