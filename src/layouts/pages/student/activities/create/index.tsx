...
const Activity = (props: any) => {
  ...
  const { values, handleChange, handleBlur, handleSubmit, setFieldValue } = useFormik({
    ...
    onSubmit: (values, action) => {
      if (editActivities) {
        axios
          .put("http://10.0.20.200:8000/mg_activity", values, {
            headers: {
        ...
        axios
          .post("http://10.0.20.200:8000/mg_activity", values, {
            headers: {
              "Content-Type": "multipart/form-data",
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleDelete = async (activity_name: string) => {
    try {
      const response = await axios.delete("http://10.0.20.200:8000/mg_activity", {
  ...
  const fetchStudentActivities = () => {
    axios
      .get(`http://10.0.20.200:8000/mg_activity?student_name=${student_guardian}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
